...
def scan_sources(
    data_dir: Path, context_dir: Path | None = None
) -> dict[str, pl.LazyFrame]:
    files = {
        "history": "sales_history.csv",
        "test": "test.csv",
        "items": "item_catalogue.csv",
        "stores": "store_info.csv",
        "events": "events_and_holidays.csv",
        "oil": "index_oil.csv",
    ...
    sources = {
        "history": pl.scan_csv(paths["history"], schema=HISTORY_SCHEMA).with_columns(
            parse_date()
        ),
        "test": pl.scan_csv(paths["test"], schema=TEST_SCHEMA).with_columns(
            parse_date()
        ),
        "items": pl.scan_csv(paths["items"], schema=ITEM_SCHEMA),
        "stores": pl.scan_csv(paths["stores"], schema=STORE_SCHEMA),
        "events": pl.scan_csv(paths["events"], schema=EVENT_SCHEMA).with_columns(
            parse_date()
        ),
    ...
    if context_dir is not None:
        traffic_path = context_dir / "store_traffic.csv"
        if not traffic_path.is_file():
            raise FileNotFoundError(f"Missing required context file: {traffic_path}")
        sources["traffic"] = pl.scan_csv(
            traffic_path, schema=TRAFFIC_SCHEMA
        ).with_columns(parse_date())
    return sources
