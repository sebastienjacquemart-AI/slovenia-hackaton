...
import polars as pl
from .config import EVENT_SCHEMA, HISTORY_SCHEMA, ITEM_SCHEMA, OIL_SCHEMA, STORE_SCHEMA, TEST_SCHEMA, TRAFFIC_SCHEMA

def parse_date(column: str = "date") -> pl.Expr:
    return pl.col(column).str.strptime(pl.Date, "%Y-%m-%d", strict=True)

def scan_sources(data_dir: Path, context_dir: Path | None = None) -> dict[str, pl.LazyFrame]:
    ...
    if context_dir is not None:
        ...
        sources["traffic"] = pl.scan_csv(traffic_path, schema=TRAFFIC_SCHEMA).with_columns(parse_date())
    return sources
