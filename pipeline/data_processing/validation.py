...
def validate_panel(
    frame: pl.LazyFrame, name: str, expected_rows: int, require_sales: bool
) -> dict[str, object]:
    expressions = [
    ...
    if require_sales:
        expressions += [
            ...
            ((~pl.col("sales_count").is_finite()) | (pl.col("sales_count") < 0))
            .fill_null(True)
            .sum()
    ...
    row_count, unique = (
        int(scalar(summary, "row_count")),
        int(scalar(summary, "unique_key_count")),
    )
    if row_count != expected_rows:
        raise ValueError(f"{name} has {row_count:,} rows; expected {expected_rows:,}")
    if unique != row_count:
        raise ValueError(f"{name} has {row_count - unique:,} duplicate keys")
...
def validate_dimension(
    ...
    if int(scalar(summary, "row_count")) != expected_rows:
        raise ValueError(
            f"{name} has {int(scalar(summary, 'row_count')):,} rows; expected {expected_rows:,}"
        )
    if int(scalar(summary, "unique_key_count")) != expected_rows:
...
def validate_sample_ids(test: pl.LazyFrame, sample: pl.LazyFrame) -> None:
    if (
        ...
        .collect()
        .get_column("id")
        .equals(sample.select("id").collect().get_column("id"))
    ):
